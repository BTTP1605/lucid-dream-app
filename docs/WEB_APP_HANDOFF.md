...
- **iOSは`volume`を無視する。** 音量の一時的な抑制には必ず `muted` を使うこと(`volume = 0` は効かず、フル音量で鳴ってしまう)。なお、この制約により**音量スライダーはiOSでは効かない**(端末の音量ボタンで調整する)。
...
### 音量スライダーが効かない(iPhone)
iOSの仕様(`volume`が無視される)。端末の音量ボタンで調整する。将来直すならWeb Audio APIのGainNodeが必要。

---

## 7. 既知の制限

- **iOSでは音量スライダーが機能しない**(上記)。
